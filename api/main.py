...
def read_one_recourses(resource_id: int, db: Session = Depends(get_db)):
...
def read_orders_detail(db: Session = Depends(get_db)):
    return order_details.read_all(db)


@app.get("/orders-details/{order_detail_id}", response_model=schemas.OrderDetail, tags=["OrderDetails"])
...
@app.put("/orders-details/{order_detail_id}", response_model=schemas.OrderDetail, tags=["OrderDetails"])
...
@app.delete("/orders-details/{order_detail_id}", tags=["OrderDetails"])
